...
export default function Flights() {
  ...
  return (
    <PublicLayout>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-sky-50 text-sky-700 rounded-full text-sm font-semibold mb-6 border border-sky-100">
                <Plane className="w-4 h-4" />
                Easy Booking
              </div>
              <h1 className="text-5xl md:text-6xl font-bold mb-6 text-gray-900 tracking-tight leading-tight">
                Book Flights to <span className="text-sky-600">Anywhere</span>
              </h1>
              <p className="text-xl text-gray-600 mb-8 leading-relaxed">
                Access 500+ airlines with the best prices. Pay with crypto or fiat. Instant confirmation.
              </p>
              <div className="flex flex-wrap gap-4">
                <Link to="/signup" className="px-8 py-4 bg-sky-600 text-white font-bold rounded-full hover:bg-sky-700 transition-all shadow-lg hover:shadow-sky-500/30 inline-flex items-center gap-2">
            ...
            {destinations.map((dest, index) => (
              <motion.div key={dest.city} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="bg-white dark:bg-gray-900 rounded-xl overflow-hidden hover:shadow-xl transition-shadow group cursor-pointer">
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 bg-white dark:bg-gray-900 px-3 py-1 rounded-full text-sm font-semibold text-sky-600 dark:text-sky-400">
                    From {dest.from}
                  </div>
                </div>
                <div className="p-6">
      ...
      {/* CTA */}
      <section className="py-20 bg-gradient-to-br from-sky-600 to-indigo-800 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold mb-6">Ready to Take Off?</h2>
          <p className="text-xl text-sky-100 mb-8">Book your next flight with JAXOPAY and enjoy the best prices and service</p>
          <Link to="/signup" className="inline-flex items-center gap-2 px-8 py-4 bg-white text-sky-600 font-semibold rounded-lg hover:bg-gray-100 transition-colors">
