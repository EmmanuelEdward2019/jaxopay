...
export default function GiftCards() {
  ...
  return (
    <PublicLayout>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-pink-50 text-pink-700 rounded-full text-sm font-semibold mb-6 border border-pink-100">
                <Gift className="w-4 h-4" />
                Gift Card Trading
              </div>
              <h1 className="text-5xl md:text-6xl font-bold mb-6 text-gray-900 tracking-tight leading-tight">
                Trade Gift Cards at the <span className="text-pink-600">Best Rates</span>
              </h1>
              <p className="text-xl text-gray-600 mb-8 leading-relaxed">
                Buy and sell gift cards from 100+ top brands. Instant verification, competitive rates, secure transactions.
              </p>
              <div className="flex flex-wrap gap-4">
                <Link to="/signup" className="px-8 py-4 bg-pink-600 text-white font-bold rounded-full hover:bg-pink-700 transition-all shadow-lg hover:shadow-pink-500/30 inline-flex items-center gap-2">
            ...
            {brands.map((brand, index) => (
              <motion.div key={brand.name} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="bg-white dark:bg-gray-900 rounded-xl overflow-hidden hover:shadow-xl transition-shadow">
                ...
                <div className="p-4 text-center">
                  ...
                  <div className="text-sm text-pink-600 dark:text-pink-400 font-semibold">Up to {brand.rate}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="py-20 bg-gradient-to-br from-pink-600 to-red-800 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold mb-6">Start Trading Gift Cards Today</h2>
          <p className="text-xl text-pink-100 mb-8">Join thousands who trust JAXOPAY for secure gift card trading</p>
          <Link to="/signup" className="inline-flex items-center gap-2 px-8 py-4 bg-white text-pink-600 font-semibold rounded-lg hover:bg-gray-100 transition-colors">
