...
const InstantSwap = () => {
  ...
  // Obiex has no lightweight "preview" endpoint, so even this fires a real quote call under the
  // hood (getExchangeRates -> getTemporarySwapQuote) against the same shared hourly allowance as
  // the actual Get Quote button. 650ms (was 300ms) meaningfully cuts how many of those a user
  // typing/backspacing through an amount generates, while still feeling responsive.
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-x-hidden">
      {/* Header */}
      <div className="bg-card border-b border-border px-3 py-4 sm:px-6">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">Instant Swap</h1>
              <p className="text-[11px] text-muted-foreground">Exchange crypto instantly at best rates</p>
            </div>
          </div>
          <div className="flex items-center gap-1 px-2 py-1 bg-success/10 rounded-lg">
            <ShieldCheck className="w-3.5 h-3.5 text-success" />
            <span className="text-[10px] font-bold text-success">Secured</span>
          </div>
        </div>
      </div>
            ...
            <button key={`${p.from}-${p.to}`}
              ...
              className={`shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-all ${
                fromCode === p.from && toCode === p.to
                  ...
                  : 'bg-card border-border text-muted-foreground hover:text-gray-900 dark:hover:text-white hover:border-gray-500 dark:hover:border-gray-400'
              }`}>
              {p.from}/{p.to}
            </button>
          ))}
        ...
        <div className="w-full max-w-lg">
          ...
          {swapPhase === 'completed' && swapResult && (
            <div className="bg-card rounded-2xl border border-border p-6 text-center space-y-4">
              ...
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Swap {swapResult.status === 'initiated' ? 'Initiated' : 'Completed'}!</h2>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-card rounded-2xl border border-border p-3 sm:p-4 space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-1">
                  <span className="text-[11px] font-bold text-muted-foreground uppercase">You Pay</span>
                  <span className="text-[11px] text-muted-foreground truncate">
                    Bal: <span className="text-gray-900 dark:text-white font-medium">{fmtBal(getBalance(fromCode), fromCode)} {fromCode}</span>
                  </span>
                </div>
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <button onClick={() => openPicker('from')}
                    ...
                    <span className="text-sm font-bold text-gray-900 dark:text-white">{fromCode}</span>
              ...
              <div className="bg-card rounded-2xl border border-border p-3 sm:p-4 space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-1">
                  <span className="text-[11px] font-bold text-muted-foreground uppercase">You Receive</span>
                  <span className="text-[11px] text-muted-foreground truncate">
                    Bal: <span className="text-gray-900 dark:text-white font-medium">{fmtBal(getBalance(toCode), toCode)} {toCode}</span>
                  </span>
                </div>
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="relative flex-1 min-w-0 flex items-center">
                  ...
                  <button onClick={() => openPicker('to')}
                    ...
                    <span className="text-sm font-bold text-gray-900 dark:text-white">{toCode}</span>
                    <ChevronDown className="w-4 h-4 text-muted-foreground" />
                  </button>
                </div>
              </div>
                  ...
                  {isQuoteActive && (
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`font-bold tabular-nums ${countdownSecs > 5 ? 'text-success' : countdownSecs > 2 ? 'text-primary' : 'text-danger'}`}>
                          {countdownSecs}s
                        </span>
                        <button onClick={handleRefresh} disabled={isRefreshing}
                          className="p-1 rounded hover:bg-muted dark:hover:bg-muted transition-colors disabled:opacity-40">
              ...
              {(swapError) && (
                <div className="bg-danger/10 border border-danger/30 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <button onClick={() => setSwapError(null)}><X className="w-3.5 h-3.5 text-danger" /></button>
                </div>
              )}
              ...
              <div className="flex items-center justify-center gap-2 py-2">
                ...
                <span className="text-[10px] text-muted-foreground">Swaps are executed instantly with best available liquidity</span>
              </div>
            </div>
          )}
        </div>
      ...
      {showPicker && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60" onClick={() => setShowPicker(false)}>
          <div className="bg-card border border-border rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[70vh] flex flex-col"
            onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-border flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white">Select {pickerSide === 'from' ? 'From' : 'To'} Token</h3>
            ...
            <div className="p-4 border-b border-border">
              <div className="relative">
                ...
                <input type="text" placeholder="Search token..." value={pickerSearch} onChange={e => setPickerSearch(e.target.value)}
                  className="w-full bg-background border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-muted-foreground focus:outline-none focus:border-[#f0b90b]" />
              </div>
            </div>
                ...
                return (
                  <button key={code} onClick={() => selectToken(code)}
                    ...
                    <div className="flex-1 text-left">
                      <p className="text-sm font-bold text-gray-900 dark:text-white">{code}</p>
                      <p className="text-[10px] text-muted-foreground">{a.name || COIN_META[code]?.name || code}</p>
                    </div>
                    <span className="text-xs text-muted-foreground tabular-nums">{fmtBal(bal, code)}</span>
                  </button>
                );
              })}
