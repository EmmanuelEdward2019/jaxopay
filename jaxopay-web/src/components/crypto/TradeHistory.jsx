  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col h-full">
      <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
        <h3 className="font-bold text-sm text-gray-900 dark:text-white">Market Trades</h3>
        <span className="text-[9px] font-bold text-gray-400 uppercase">{trades.length} trades</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <table className="w-full text-[10px] text-left border-collapse">
          <thead className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-10">
            <tr>
              <th className="px-3 py-2 font-bold text-gray-400 uppercase tracking-wider">Price</th>
              <th className="px-3 py-2 font-bold text-gray-400 uppercase tracking-wider">Amount</th>
              <th className="px-3 py-2 font-bold text-gray-400 uppercase tracking-wider text-right">Time</th>
            </tr>
          </thead>
            ...
            {loading ? (
               [...Array(15)].map((_, i) => (
                <tr key={i} className="animate-pulse">
                  <td className="px-3 py-2 h-5 bg-gray-100 dark:bg-gray-700 rounded"></td>
                  <td className="px-3 py-2 h-5 bg-gray-100 dark:bg-gray-700 rounded mx-1"></td>
                  <td className="px-3 py-2 h-5 bg-gray-100 dark:bg-gray-700 rounded"></td>
                </tr>
              ))
            ) : trades.map((trade, i) => (
              <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors cursor-pointer">
                <td className={`px-3 py-2 font-bold ${trade.side === 'buy' || trade.side === 'bid' ? 'text-green-500' : 'text-red-500'}`}>
                ...
                <td className="px-3 py-2 text-gray-500 text-right font-mono">
                  {new Date(trade.created_at || trade.timestamp).toLocaleTimeString([], { 
                    hour: '2-digit', 
                    minute: '2-digit', 
                    second: '2-digit' 
                  })}
